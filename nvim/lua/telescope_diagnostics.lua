-- this is a copy of https://github.com/nvim-telescope/telescope.nvim/blob/f838695459d75a889ae8d96e370957f0f675902a/lua/telescope/builtin/__diagnostics.lua
-- with added custom filter
...
local diagnostics_to_tbl = function(opts)
  ...
  local filter_diag = function(diagnostic)
    ...
    local is_bin = string.find(filename, "/bin/")
    local is_obj = string.find(filename, "/obj/")
    local is_nuget = string.find(filename, "/.nuget/")

    return root_dir_test and 
        listed_test and 
        not is_bin and 
        not is_obj and
        not is_nuget
